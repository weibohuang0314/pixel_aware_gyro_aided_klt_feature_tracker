    ...
    deltaT(time)
{
    ...
    {
        deltaR = I + W*sin(d)/d + W*W*(1.0f-cos(d))/d2;             // on-manifold equation (3)
        rightJ = I - W*(1.0f-cos(d))/d2 + W*W*(d-sin(d))/(d2*d);    // on-manifold equation (8)
    }
}
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration, const cv::Point3f &angVel, const float &dt)
{
    ...
    // Compute rotation parts of matrices A and B (rely on non-updated delta rotation)
    A.rowRange(0,3).colRange(0,3) = dRi.deltaR.t();
    A.rowRange(3,6).colRange(0,3) = -dR*dt*accHat;
    A.rowRange(6,9).colRange(0,3) = -0.5f*dR*dt*dt*accHat;
    A.rowRange(6,9).colRange(3,6) = cv::Mat::eye(3,3,CV_32F)*dt;
    ...
    dP = dP + dV*dt + 0.5f*dR*acc*dt*dt;
    dV = dV + dR*acc*dt;
    dR = NormalizeRotation(dR*dRi.deltaR);  // Update delta rotation
...
void Calib::Set(const cv::Mat &Tbc_, const float &ng, const float &na, const float &ngw, const float &naw, const int &fps_)
{
    Tbc = Tbc_.clone();
